...
## Scope & Acceptance
- [ ] Root `Cargo.toml` with `[workspace]` and empty-but-compiling member crates:
...
- [ ] Each crate has `#![deny(missing_docs)]` and a one-line crate doc so the lint passes empty.
- [ ] Toolchain: `rust-toolchain.toml` pins a rustup channel; document `rustup target add
